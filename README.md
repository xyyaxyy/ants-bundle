...
antsxdp 定制构建工具，基于 esbuild 及 swc 实现
...
```bash
# 建议当前项目中安装
pnpm i ants-bundle  -D

# 也可以全局安装，但不推荐
pnpm i ants-bundle -g
```
...
```bash
ants-bundle src/index.ts src/cli.ts
```
...
ants-bundle --entry src/index.ts --entry src/cli.ts
```
...
ants-bundle --entry.foo src/index.ts --entry.bar src/cli.ts
```

也可以在 `ants-config.ts` 中配置：
...
```bash
ants-bundle index.ts --sourcemap
```
...
```bash
ants-bundle src/index.ts --format esm,cjs,iife
```
...
```bash
ants-bundle src/index.ts --format esm,cjs,iife --legacy-output
```
...
```bash
ants-bundle src/index.ts --watch
```

启动`watch`模式，这意味着在初始构建后，ants-bundle 会监听文件变化。

可以使用`--ignore-watch`来取消指定文件的监听。

```bash
ants-bundle src src/index.ts --watch --ignore-watch folder1 --ignore-watch folder2
```

## 成功回调

```bash
ants-bundle src/index.ts --watch --onSuccess "node dist/index.js"
```
...
```bash
ants-bundle src/index.ts --minify
```
