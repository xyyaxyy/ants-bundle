...
import { getAllDepsHash, loadAntsBundleConfig } from './load';
...
const normalizeOptions = async (
  ...
  if (!entry || Object.keys(entry).length === 0) {
    throw new PrettyError(`No input files, try "ants-bundle <your-file>" instead`);
  }
...
export async function build(_options: Options) {
  ...
  await Promise.all(
    [...(Array.isArray(configData) ? configData : [configData])].map(async (item) => {
      ...
      logger.info('CLI', `ants-bundle v${version}`);

      if (config.path) {
        logger.info('CLI', `Using ants-bundle config: ${config.path}`);
      }
